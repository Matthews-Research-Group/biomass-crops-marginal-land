...
weach.default <- function(X, latitude, ts=1, ...){
  ...
  if (dim(X)[2] != 12)
    stop("X should have 12 columns")

  sfc = 101325 # surface pressure (Pa)  
  ...
  WindSpeed <- X[, 10]
  precip <- X[, 11]
  sph <- X[, 12]  #specific humidity (kg/kg)
...
#Yufeng +
  #I suppose temp in celcius? Convert it to Kelvin
  temp_K = temp + 273.15
  # Assume no diurnal variation in sph and sfc
  rh=0.01*0.263*sfc*sph*exp(17.67*(273.15-temp_K)/(temp_K-29.65))-sph*0.6077
  rh[rh>1.0]= 1.0  #make sure no values exceed 1
  rh[rh<0]  = 0   #make sure no values below 0 
#Yufeng -
